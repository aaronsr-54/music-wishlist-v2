...
@Component({
  ...
  imports: [DatePipe, CoverComponent, TypeChipComponent, PreviewSpinnerComponent, IconComponent],
  template: `
    <div class="flex flex-col p-2 rounded-md bg-ink-200 gap-1 w-full overflow-hidden">
      <button
        class="relative w-full border-none bg-transparent p-0 cursor-pointer shrink-0 rounded-sm transition-opacity duration-fast ease-smooth disabled:cursor-not-allowed disabled:opacity-60 enabled:hover:opacity-80"
      ...
      <div class="flex flex-col gap-2 min-w-0">
        <div class="flex items-center justify-between text-bone-800">
          <span class="font-display text-[clamp(0.75rem,0.6457rem+0.4049vw,1rem)] text-bone-600 flex gap-0.5 items-center">
            <b class="font-bold not-italic text-bone-700">{{ releaseItem().releaseDate | date: 'dd' }}</b>
            <em class="italic font-normal text-bone-600 lowercase">{{ releaseItem().releaseDate | date: 'LLL' }}</em>
          </span>
          <app-type-chip [type]="releaseItem().type" />
        </div>

        <div class="flex flex-col gap-1">
          <span class="font-display text-[clamp(0.875rem,0.7707rem+0.4049vw,1.125rem)] font-semibold text-bone-100 leading-none truncate max-w-full">
            {{ releaseItem().name }}
          </span>
          <span class="text-[clamp(0.75rem,0.6457rem+0.4049vw,1rem)] text-bone-600 truncate max-w-full">
            {{ releaseItem().artist }}
          </span>
        </div>
