...
export default async (req, res) => {
  ...
  try {
    ...
    if (!subDoc.exists) {
      return res.status(400).json({ error: 'No push subscription found. Subscribe first.' });
    }
    ...
    if (favSnapshot.empty) {
      return res.status(400).json({ error: 'No favorite artists found. Add some first.' });
    }
    ...
    for (const favorite of favorites) {
      ...
      for (const album of newAlbums) {
        ...
        try {
          ...
          notified++;
          artistResults.albums.push({
            id: album.id,
            title: album.title,
            recordType: releaseType,
            sent: true,
          });
        } catch (err) {
    ...
    console.error('check-releases-debug error:', error);
